// screen-catalog.jsx — Skill Catalog
// Layout: filter bar → table (left 2/3) + selected skill preview (right 1/3).

function ScreenCatalog({ goto, focusId, setFocusId }) {
  const { SKILLS } = window.SAVANT;
  ...
  const filtered = React.useMemo(() => {
    ...
    if (query) {
      ...
      list = list.filter(s => s.name.toLowerCase().includes(q) || s.owner.toLowerCase().includes(q) || s.team.toLowerCase().includes(q));
    }
    return list;
  }, [SKILLS, tierFilter, statusFilter, query]);

  const selected = SKILLS.find(s => s.id === focusId) || filtered[0] || SKILLS[0];

  const tierCount = (t) => SKILLS.filter(s => s.tier === t).length;
  const statusCount = (k) => {
      ...
      <div className="page-head">
        <div>
          ...
          <div className="page-head-sub">
            Every governed skill, with provenance back to its source repository.
            Browse, filter, or open a skill to inspect its evaluation history.
          </div>
        </div>
        <div className="row" style={{ gap: 8 }}>
          <button className="btn btn-ghost"><Ic.Sort className="b-icon" />Export catalog</button>
          <button className="btn btn-primary"><Ic.Plus className="b-icon" />New skill</button>
        </div>
      </div>
      ...
      <div className="split" style={{ alignItems: "flex-start" }}>
        ...
        <div className="col" style={{ gap: 0 }}>
          <div className="filterbar">
            ...
            <div className="row" style={{ gap: 6 }}>
              <Facet active={statusFilter === "all"} onClick={() => setStatusFilter("all")}>All channels</Facet>
            ...
            <div style={{ position: "relative" }}>
              <Ic.Search style={{ width: 12, height: 12, color: "var(--subtle)", position: "absolute", left: 8, top: "50%", transform: "translateY(-50%)" }} />
              <input
                value={query}
                onChange={e => setQuery(e.target.value)}
                placeholder="Search skills, owners, teams…"
                style={{
                  height: 26, padding: "0 10px 0 26px", fontSize: 12, border: "1px solid var(--rule-2)",
                  borderRadius: 4, background: "var(--panel)", width: 220, outline: "none"
                }}
              />
            </div>
          </div>
                ...
                <tbody>
                  {filtered.map(s => (
                    ...
                    >
                      ...
                      <td><Tier n={s.tier} /></td>
                      <td><RepoChip provider={s.repoProvider} name={s.repo} /></td>
                      <td>
                        {s.ref !== "—"
                          ? <span className="mono num" style={{ color: "var(--ink-3)" }}>{s.ref}</span>
                          : <span className="subtle mono">—</span>}
                      </td>
                      <td><EnvPill env={s.channel} /></td>
                      <td className="muted" style={{ fontSize: 12.5 }}>{s.owner}</td>
        ...
        {/* RIGHT: preview panel */}
        {selected ? <CatalogPreview skill={selected} goto={goto} /> : null}
      </div>
    </div>
  );
}

function CatalogPreview({ skill, goto }) {
  return (
    <div className="panel" style={{ position: "sticky", top: 0 }}>
      <div className="panel-hd">
        <div className="panel-title">Preview</div>
        <button className="icon-btn"><Ic.More style={{ width: 14, height: 14 }} /></button>
      </div>
      <div className="panel-bd" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        <div>
          ...
          <div className="h1" style={{ fontSize: 18 }}>{skill.name}</div>
          <div className="muted" style={{ fontSize: 12.5, marginTop: 6, lineHeight: 1.45 }}>{skill.description}</div>
        </div>
        ...
        <div className="col" style={{ gap: 6 }}>
          ...
          <PreviewRow label="Production">
            {skill.ref !== "—" ? <CommitRef commit={skill.commit} label={skill.ref} /> : <span className="subtle">No production version</span>}
          </PreviewRow>
          {skill.candidateRef !== "—" && (
          ...
          <PreviewRow label="Owner">
            <div className="row" style={{ gap: 6 }}>
              ...
              <span className="muted" style={{ fontSize: 11.5 }}>· {skill.team}</span>
            </div>
          </PreviewRow>
          <PreviewRow label="Access">
            <span className="ref"><Ic.Lock className="ref-icon" /> {skill.accessGroup}</span>
          </PreviewRow>
          <PreviewRow label="Versions">
            <span style={{ fontSize: 12.5, color: "var(--ink-2)" }}>{skill.versionCount} on record</span>
          </PreviewRow>
        </div>
        ...
        {skill.trend.length > 0 ? (
          <div>
            <div className="eyebrow" style={{ marginBottom: 8 }}>Score trend · last 10 evals</div>
            <div className="row" style={{ gap: 10 }}>
              <div className="h-display" style={{ fontSize: 32 }}>{skill.score.toFixed(1)}</div>
          ...
          <div className="note">
            ...
            <span>No evaluation history yet. Candidate <span className="mono">{skill.candidateRef}</span> is the first release.</span>
          </div>
        )}

        <div className="row" style={{ gap: 8 }}>
          <button className="btn btn-primary grow" onClick={() => goto("skill", skill.id)}>
            <span>Open skill</span>
            <Ic.ChevR className="b-icon" />
          </button>
          <button className="btn btn-ghost"><Ic.ExternalLink className="b-icon" /></button>
        </div>
      </div>
    </div>
  );
