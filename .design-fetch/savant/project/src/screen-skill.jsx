// screen-skill.jsx — Skill Detail + Evaluation
// Hero: skill identity row → PROVENANCE RAIL (the signature motif).
// Below: tabs (Evaluation / Versions / Access / Activity).
// Default tab: Evaluation — A/B baseline vs candidate, rubric bars, regressions, timeline, comments.

function ScreenSkill({ skillId, goto }) {
  const { SKILLS, RUBRIC_BASELINE, APPROVAL_TIMELINE, COMMENTS } = window.SAVANT;
  const skill = SKILLS.find(s => s.id === skillId) || SKILLS[0];
  const [tab, setTab] = React.useState("evaluation");

  // Construct provenance steps for this skill
  const railSteps = [
    {
      label: "Repository",
      value: skill.repo,
      meta: <span className="row" style={{ gap: 4 }}>
    ...
    },
    {
      ...
      meta: <span className="mono subtle" style={{ fontSize: 10.5 }}>
    ...
    {
      label: "Evaluation",
      value: skill.status.startsWith("candidate-running") ? "Running · 184 / 248" : "248 cases · 6 flagged",
      meta: <span className="muted">Baseline {skill.score?.toFixed(1) ?? "—"} → cand {(skill.score + 0.6).toFixed(1)}</span>,
      state: skill.status.startsWith("candidate-running") ? "warn" : "warn",
    },
    {
      label: "Approval",
    ...
    },
    {
      ...
      meta: <span className="muted">{skill.channel === "production" ? "Pinned 2d ago" : "Promote after approval"}</span>,
  ...
  return (
    <div className="page-inner">
      ...
      <div className="page-head">
        <div style={{ minWidth: 0 }}>
          <div className="page-head-meta">
            <span onClick={() => goto("catalog")} style={{ cursor: "default" }} className="link">Catalog</span>
        ...
        <div className="row" style={{ gap: 8 }}>
          <button className="btn btn-ghost"><Ic.ExternalLink className="b-icon" />View in {skill.repoProvider}</button>
          <button className="btn btn-danger">Reject candidate</button>
          <button className="btn btn-brass"><Ic.Check className="b-icon" />Approve for release</button>
        </div>
      </div>
      ...
      {tab === "evaluation" && <EvalTab skill={skill} rubric={RUBRIC_BASELINE} timeline={APPROVAL_TIMELINE} comments={COMMENTS} />}
      {tab === "versions"   && <VersionsTab skill={skill} />}
      {tab === "access"     && <AccessTab skill={skill} />}
      {tab === "activity"   && <ActivityTab />}
    </div>
  );
}

function EvalTab({ skill, rubric, timeline, comments }) {
      ...
      <div className="col" style={{ gap: "var(--gutter)", minWidth: 0 }}>
        ...
        <div className="note brass">
          ...
          <div className="grow">
            ...
            <div style={{ fontSize: 12, color: "var(--brass-deep)", opacity: .8, marginTop: 2 }}>
              Eval set: <span className="mono">contract-corpus-v9.eval</span> · 248 cases · ran 58m ago · 24s
            </div>
          </div>
          <button className="btn btn-sm" style={{ background: "var(--panel)", borderColor: "var(--rule-2)" }}>
        ...
        <div className="ab">
          <div className="ab-side">
            ...
            <div className="row" style={{ gap: 8 }}>
              ...
              <span className="subtle" style={{ fontSize: 11.5 }}>shipped 6d ago</span>
            </div>
            <div style={{ marginTop: 12 }}>
              <div className="row" style={{ gap: 14, marginBottom: 8 }}>
                <Score label="Overall" value={skill.score} />
  ...
  "recommend": "approve"
          ...
          <div className="ab-side">
            ...
            <div className="row" style={{ gap: 8 }}>
              ...
              <span className="subtle" style={{ fontSize: 11.5 }}>by {skill.owner} · 1h ago</span>
            </div>
            <div style={{ marginTop: 12 }}>
              <div className="row" style={{ gap: 14, marginBottom: 8 }}>
                <Score label="Overall" value={skill.score + overallDelta} delta={overallDelta} />
        ...
        <div className="panel">
          <div className="panel-hd">
            ...
            <div className="row" style={{ gap: 6 }}>
              ...
              <span className="row" style={{ gap: 5, fontSize: 11.5, color: "var(--muted)", marginLeft: 12 }}>
                <span style={{ width: 8, height: 6, background: "var(--moss)", borderRadius: 2 }} /> candidate
              </span>
            </div>
          </div>
        ...
        <div className="panel">
          <div className="panel-hd">
            ...
            <div className="row" style={{ gap: 6 }}>
              ...
              <button className="btn btn-sm">Open in eval ↗</button>
            </div>
          </div>
          <div className="panel-bd tight">
            <table className="tbl">
              ...
              <tbody>
                <FlagRow id="0117" desc="Mutual NDA with non-standard indemnity"      rubric="Clause extraction precision" b={1.00} c={0.55} />
                ...
                <FlagRow id="0226" desc="Non-standard governing law"                   rubric="Clause extraction precision" b={0.90} c={0.79} />
      ...
      <div className="col" style={{ gap: "var(--gutter)" }}>
        ...
        <div className="panel">
          <div className="panel-hd">
            ...
            <span className="chip chip-brass"><span className="dot" />awaiting compliance</span>
          </div>
          <div className="panel-bd">
            <div className="tl">
              {APPROVAL_TIMELINE_RENDER(timeline)}
            </div>
          </div>
        </div>
        ...
        <div className="panel">
          <div className="panel-hd">
            ...
            <span className="subtle" style={{ fontSize: 11.5 }}>2 of 3</span>
          </div>
          <div className="panel-bd tight">
        ...
        <div className="panel">
          <div className="panel-hd">
            ...
            <span className="subtle" style={{ fontSize: 11.5 }}>{COMMENTS.length}</span>
          </div>
          <div className="panel-bd" style={{ display: "flex", flexDirection: "column", gap: 14 }}>
            {window.SAVANT.COMMENTS.map((c, i) => (
              <div key={i} className="col" style={{ gap: 6 }}>
                ...
                <div style={{ fontSize: 12.5, color: "var(--ink-2)", lineHeight: 1.5, paddingLeft: 28 }}>{c.text}</div>
              </div>
            ))}
            <textarea
...
function FlagRow({ id, desc, rubric, b, c }) {
  const delta = c - b;
  return (
    <tr>
      <td><span className="mono num">#{id}</span></td>
      <td>{desc}</td>
      <td className="muted" style={{ fontSize: 12 }}>{rubric}</td>
      <td className="mono num" style={{ textAlign: "right" }}>{b.toFixed(2)}</td>
      <td className="mono num" style={{ textAlign: "right", color: "var(--oxblood)" }}>{c.toFixed(2)}</td>
      <td style={{ textAlign: "right" }}><Delta v={delta * 100} /></td>
    </tr>
  );
}

function Score({ label, value, delta, unit, worseUp }) {
  ...
  return (
    <div className="col" style={{ gap: 2 }}>
      ...
      <div className="row" style={{ gap: 6, alignItems: "baseline" }}>
        <span className="h-display" style={{ fontSize: 20 }}>{typeof value === "number" ? value.toFixed(1) : value}<span style={{ fontSize: 12, color: "var(--muted)" }}>{unit || ""}</span></span>
        {delta != null && (
...
function ApprovalRow({ role, who, status, when }) {
  return (
    <div style={{
      display: "grid",
      ...
      <div>
        ...
        <div className="muted" style={{ fontSize: 11.5 }}>{who === "—" ? "Awaiting reviewer assignment" : who}</div>
      </div>
      <div className="row" style={{ gap: 8 }}>
        {status === "approved" ? (
          <span className="chip chip-moss"><Ic.Check style={{ width: 10, height: 10 }} />Approved</span>
        ) : (
          <span className="chip chip-brass"><Ic.Clock style={{ width: 10, height: 10 }} />Pending</span>
        )}
        <span className="subtle" style={{ fontSize: 11.5, width: 56, textAlign: "right" }}>{when}</span>
      </div>
    </div>
  );
}

function APPROVAL_TIMELINE_RENDER(timeline) {
  return timeline.map((e, i) => (
    <div className="tl-item" key={i}>
      <span className={`tl-node ${e.node}`} />
      <div>
        <div className="tl-pri"><b>{e.who}</b> <span className="muted">{e.action}</span></div>
  ...
  return (
    <div className="panel">
      <div className="panel-hd">
        ...
        <div className="row" style={{ gap: 6 }}>
          <button className="btn btn-sm">Compare versions</button>
      ...
      <div className="panel-bd tight">
        <table className="tbl">
          ...
          <tbody>
            {versions.map((v, i) => (
              <tr key={i}>
                <td><span className="mono num" style={{ color: "var(--ink)", fontWeight: 500 }}>{v.ref}</span></td>
                <td><CommitRef commit={v.commit} /></td>
                <td className="muted">{v.who}</td>
                <td>
                  {v.channel === "candidate"  ? <span className="chip chip-brass">candidate</span>
                  : v.channel === "production" ? <EnvPill env="production" />
                  : <span className="chip chip-paper">archived</span>}
                </td>
                <td className="subtle">{v.when}</td>
                <td className="mono num" style={{ textAlign: "right" }}>{v.score.toFixed(1)}</td>
                <td style={{ textAlign: "right" }}><Delta v={v.delta} /></td>
    ...
    <div className="grid-2" style={{ alignItems: "flex-start" }}>
      <div className="panel">
        <div className="panel-hd">
          ...
          <button className="btn btn-sm"><Ic.Plus className="b-icon" />Add group</button>
        </div>
        <div className="panel-bd tight">
          <table className="tbl">
            <thead>
              <tr><th>Group</th><th>Members</th><th>Permissions</th><th>Source</th><th style={{ textAlign: "right" }}>Last sync</th></tr>
            </thead>
            <tbody>
              {groups.map((g, i) => (
                <tr key={i}>
                  <td><span className="mono" style={{ color: "var(--ink)" }}>{g.name}</span></td>
                  <td className="num">{g.members}</td>
                  <td className="muted">{g.perm}</td>
                  <td><span className="chip chip-paper">{g.source}</span></td>
                  <td className="subtle" style={{ textAlign: "right" }}>{g.last}</td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="panel-bd" style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {pols.map((p, i) => (
            <div key={i} style={{
              ...
              <div className="eyebrow" style={{ fontSize: 10, marginBottom: 3 }}>{p.rule}</div>
  ...
  return (
    <div className="panel">
      <div className="panel-hd">
        ...
        <button className="btn btn-sm">Export log</button>
      </div>
      <div className="panel-bd">
        <div className="tl">
          {events.map((e, i) => (
            <div className="tl-item" key={i}>
              <span className={`tl-node ${e.node}`} />
              <div>
                <div className="tl-pri"><b>{e.who}</b> <span className="muted">{e.action.toLowerCase()}</span> {e.target}</div>
              </div>
              <div className="tl-meta">{e.when}</div>
            </div>
          ))}
...
window.ScreenSkill = ScreenSkill;
