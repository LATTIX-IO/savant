# Copilot Instructions for lattix-monorepo

## Repo role

This repository is a monorepo-style git superproject that coordinates changes across many child repositories checked out as submodules. Treat it as an orchestration workspace, not a single standalone codebase.

## Simplified submodule workflow

If a task changes code inside a submodule, make that change in the child repository directly and update the monorepo gitlink only when the superproject should point at the new child commit.

Do **not** assume child repositories must reuse the monorepo branch name.
Do **not** auto-create child PRs from the monorepo.
Do **not** generate a fanout manifest or require one for submodule pointer changes.

## CI policy

- Preserve `pull_request` CI for pre-merge validation
- Preserve `push` to `main` CI for post-merge and direct-fix validation
- Keep monorepo CI focused on visibility and validation rather than cross-repo orchestration

## Source of truth

- `.github/workflows/submodule-governance.yml`
- `.gitmodules`

If a task is monorepo-only and touches no submodule paths, handle it entirely inside this repository.
