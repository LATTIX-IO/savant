...
import { POLICIES, type PolicyType } from "@/lib/savant-data";

type Filter = "all" | PolicyType;

export function PoliciesScreen() {
  const [selId, setSelId] = useState<string>(POLICIES[0]!.id);
  ...
  const typeCount = (t: PolicyType) => POLICIES.filter((p) => p.type === t).length;
      ...
      <div className="page-head">
        <div>
          ...
          <div className="page-head-sub">
            Codified rules for access, approvals, distribution, and environment promotion. Policies
            are applied to skills by tier or by tag.
          </div>
        </div>
              ...
              {filtered.map((p) => (
                ...
                >
                  ...
                  <div className="row" style={{ gap: 16, fontSize: 11.5 }}>
                    <span>
                      <span className="subtle">Affects</span>{" "}
            ...
            <div>
              ...
              <div className="col" style={{ gap: 6 }}>
                <EnforceLine
                  ...
                  action="Blocked"
                  who="renata.m"
                  detail="attempted to distribute Tier 1 skill via vscode-sync"
                />
                <EnforceLine
                  when="3h ago"
                  action="Allowed"
                  who="ari.chen"
                  detail="approved promotion of Engineering RFC Reviewer"
                />
                <EnforceLine
                  when="6h ago"
                  action="Allowed"
                  who="jdv"
                  detail="rollback Incident Triage → v2.9.4 with break-glass"
                />
              </div>
            </div>
          </div>
...
function EnforceLine({
  ...
  return (
    <div className="row" style={{ gap: 10, fontSize: 12, padding: "5px 0" }}>
      ...
      <span
        className={`chip ${action === "Blocked" ? "chip-blood" : "chip-moss"}`}
